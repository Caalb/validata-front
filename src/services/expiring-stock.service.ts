...
class ExpiringStockService {
   ...
   */
  async getExpiringStocks(): Promise<ExpiringStockItem[]> {
    try {
      const response = await api.get<ExpiringStockItem[]>('/stocks')
      return response.data
    } catch (error) {
      console.error('Erro ao buscar estoques para expiração:', error)
      throw error
    }
  }
  ...
  async getWeeklyExpirationAnalytics(weekOffset: number = 0): Promise<WeeklyExpirationData> {
    try {
      const allStocks = await this.getExpiringStocks()
...
export const expiringStockService = new ExpiringStockService()
